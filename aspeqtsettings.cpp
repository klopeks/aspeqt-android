...
#ifdef Q_OS_ANDROID
// Write the in-app language code to <filesDir>/aspeqt_lang.txt so the Java
// side (LocaleHelper) can wrap Activity context with the same locale.
// Without this, Toasts and dialogs from Java always follow the Android
// system locale even when the user switches language in AspeQt Options.
static void writeAndroidLangFile(const QString &lang)
{
    QString dir = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
    if (dir.isEmpty()) return;
    QDir().mkpath(dir);
    QFile f(dir + QStringLiteral("/aspeqt_lang.txt"));
...
AspeqtSettings::AspeqtSettings()
{
    ...
    // Default handshake: CTS (2). Empirically the most reliable for Lotharek's
    // and Ray's SIO2PC-USB cables — matches what works on Windows AspeQt.
    // Original default was 3 (SOFT) which was a poor fit for any USB FTDI cable.
    mSerialPortHandshakingMethod = mSettings->value("HandshakingMethod", 2).toInt();

    // Sanity fix: if user previously got stuck on SOFT while using USB SIO2PC,
    // auto-correct to CTS on this run. SOFT only makes sense for the SIO2BT path.
    if (mSerialPortInterface == 0 && mSerialPortHandshakingMethod == 3) {
        mSerialPortHandshakingMethod = 2;
        mSettings->setValue("HandshakingMethod", 2);
    }
...
#ifdef Q_OS_ANDROID
    writeAndroidLangFile(mI18nLanguage);
#endif
