    ...
    : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    // Note: do NOT log "AspeQt started" here — translators are not yet loaded
    // and the message would always appear in English. It is logged after
    // loadTranslators() below.
    ...
    /* Log start banner now that translators are active (so the timestamp
       message appears in the user's language). Use QLocale::system() so the
       date itself ("Mon", "May", etc.) is also localised — QDateTime::toString()
       without a locale uses C-style English regardless of installed translators. */
    qDebug() << "!d" << tr("AspeQt started at %1.")
    ...
    for (int i = 0; i < g_numberOfDisks; i++) {
        diskRowContainers[i] = nullptr;
        mountedFileLabels[i] = nullptr;
    }
...
#ifdef Q_OS_ANDROID
    // Auto-start SIO emulation right after the UI finishes initializing.
    // Matches Windows AspeQt's behaviour where the ▶ button does not need
    // to be pressed manually for the first session.
    QTimer::singleShot(0, this, [this]() {
        if (!ui->actionStartEmulation->isChecked()) {
            ui->actionStartEmulation->setChecked(true);
...
void MainWindow::setupAndroidDiskFileLabels()
{
#ifdef Q_OS_ANDROID
    ...
    for (int i = 0; i < g_numberOfDisks; i++) {
        int index = grid->indexOf(diskWidgets[i].frame);
        if (index < 0) {
            continue;
        }
        ...
        grid->removeWidget(diskWidgets[i].frame);
        ...
        containerLayout->addWidget(diskWidgets[i].frame);

        mountedFileLabels[i] = new QLabel(container);
        mountedFileLabels[i]->setObjectName(QString("mountedFileLabel_%1").arg(i + 1));
        mountedFileLabels[i]->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);
        mountedFileLabels[i]->setIndent(40);
        mountedFileLabels[i]->setWordWrap(false);
        mountedFileLabels[i]->setTextFormat(Qt::PlainText);
        mountedFileLabels[i]->setVisible(false);
        containerLayout->addWidget(mountedFileLabels[i]);

        diskWidgets[i].fileNameLabel->setVisible(false);
        diskWidgets[i].imagePropertiesLabel->setVisible(false);

        diskRowContainers[i] = container;
        grid->addWidget(container, row, column, rowSpan, columnSpan);
    }
#endif
}

void MainWindow::updateMountedFileLabel(int no, const QString &text, const QString &styleSheet,
                                        const QString &toolTip, const QString &statusTip)
{
#ifdef Q_OS_ANDROID
    if (no < 0 || no >= g_numberOfDisks || !mountedFileLabels[no]) {
        return;
    }

    mountedFileLabels[no]->setText(text);
    mountedFileLabels[no]->setStyleSheet(styleSheet);
    mountedFileLabels[no]->setToolTip(toolTip);
    mountedFileLabels[no]->setStatusTip(statusTip.isEmpty() ? toolTip : statusTip);
    mountedFileLabels[no]->setVisible(!text.isEmpty());
#else
    Q_UNUSED(no)
    Q_UNUSED(text)
    Q_UNUSED(styleSheet)
...
void MainWindow::dropEvent(QDropEvent *event)
{
    ...
    if (type == FileTypes::Cas) {
        restartForCassetteOpen();
        playCassetteImage(files.at(0));
        return;
    }
...
void MainWindow::on_actionToggleMiniMode_triggered()
{
    if(g_miniMode){
        ...
        for (int i = 1; i < g_numberOfDisks; i++) {
            if (diskRowContainers[i]) {
                diskRowContainers[i]->setVisible(true);
            }
            diskWidgets[i].frame->setVisible(true);
        }
        ...
        for (int i = 1; i < g_numberOfDisks; i++) {
            if (diskRowContainers[i]) {
                diskRowContainers[i]->setVisible(false);
            }
            diskWidgets[i].frame->setVisible(false);
        }
    ...
    if (deviceNo >= 0x31 && deviceNo <= 0x36) { //
        ...
        if (img) {
            ...
            if (img->description() == tr("Folder image")) {
                ...
                updateMountedFileLabel(deviceNo - 0x31, filenamelabel,
                                       "color: rgb(54, 168, 164); font-weight: bold;",
                                       img->originalFileName(), img->description());
                ...
                updateMountedFileLabel(deviceNo - 0x31, filenamelabel,
                                       "color: rgb(230, 230, 230); font-weight: bold;",
                                       img->originalFileName(), img->description());
                diskWidgets[deviceNo - 0x31].editAction->setEnabled(true);
            ...
            updateMountedFileLabel(deviceNo - 0x31, QString(), QString(), QString(), QString());
...
void MainWindow::changeFonts()
{
    if (aspeqtSettings->useLargeFont()) {
        ...
        for (int i = 0; i < g_numberOfDisks; i++) {
            if (mountedFileLabels[i]) {
                mountedFileLabels[i]->setFont(font);
            }
        }
...
int MainWindow::containingDiskSlot(const QPoint &point)
{
    ...
    for (i=0; i < g_numberOfDisks; i++) {    //
        QWidget *hitTarget = diskRowContainers[i] ? diskRowContainers[i] : static_cast<QWidget*>(diskWidgets[i].frame);
        QRect rect(hitTarget->mapTo(this, QPoint(0, 0)), hitTarget->size());
...
void MainWindow::prepareForXexBoot()
{
    if (ui->actionStartEmulation->isChecked()) {
        ui->actionStartEmulation->trigger();
        sio->wait();
...
void MainWindow::restartForXexOpen()
{
    if (ui->actionStartEmulation->isChecked()) {
        ui->actionStartEmulation->trigger();
        sio->wait();
...
void MainWindow::restartForCassetteOpen()
{
    if (ui->actionStartEmulation->isChecked()) {
        ui->actionStartEmulation->trigger();
        sio->wait();
...
void MainWindow::chooseAndBootExe()
{
...
void MainWindow::bootExe(const QString &fileName)
{
    ...
    if (dlg.openRequested()) {
        restartForXexOpen();
        chooseAndBootExe();
        return;
    }
    if (dlg.reloadRequested() && !g_exefileName.isEmpty()) {
        bootExe(g_exefileName);
    }
}
...
#ifdef Q_OS_ANDROID
        // Mounting a new image while emulation is running:
        // cycle the SIO worker so the FTDI port is reopened and any stale
        // command-frame state from the previous image is cleared. Skips on
        // session-restore mounts (sio not yet running on app startup).
        //
        // Going through the action's trigger (rather than calling sio->wait()
        // and sio->start() directly) keeps the QAction's checked state in
        // sync with the worker. Doing it manually leaves the action unchecked
        // after the cycle (sioFinished() unchecks it) which silently breaks
        // the next manual Stop click.
        if (sio->isRunning() && ui->actionStartEmulation->isChecked()) {
            ui->actionStartEmulation->setChecked(false);
            on_actionStartEmulation_triggered();   // stop
            qApp->processEvents();
            ui->actionStartEmulation->setChecked(true);
            on_actionStartEmulation_triggered();   // start
        }
#endif
    }
}
...
{
    // Remove ALL three translators (qt, qtbase, aspeqt). The original code
    // had a bug: it loaded qt_XX into aspeqt_qt_translator then immediately
    // OVERWROTE it with qtbase_XX, so the qt_*.qm part was lost. Use a
    // dedicated translator for each.
    qApp->removeTranslator(&aspeqt_qt_translator);
    qApp->removeTranslator(&aspeqt_qtbase_translator);
    qApp->removeTranslator(&aspeqt_translator);

    // Try to translate first the short locale code (e.g. "fr") and then the
    // full one ("fr_FR"). System locale comes as "fr_FR"; manual selection
    // gives just "fr".
    auto loadOne = [](QTranslator &t, const QString &base, const QString &loc) {
        if (!t.load(QStringLiteral(":/translations/i18n/") + base + loc)) {
            // Try just the language part (strip _COUNTRY).
            QString lang = loc.split('_').first();
            t.load(QStringLiteral(":/translations/i18n/") + base + lang);
        }
    };
    ...
    if (aspeqtSettings->i18nLanguage().compare("auto") == 0) {
        ...
        return; // English uses source strings, no translator needed.
    } else {
        lang = aspeqtSettings->i18nLanguage();
    }

    loadOne(aspeqt_translator,        "aspeqt_", lang);
    loadOne(aspeqt_qt_translator,     "qt_",     lang);
    loadOne(aspeqt_qtbase_translator, "qtbase_", lang);

    // Install in order so qtbase wins over qt for overlapping keys, and
    // aspeqt wins over both for app-specific strings.
    qApp->installTranslator(&aspeqt_qt_translator);
    qApp->installTranslator(&aspeqt_qtbase_translator);
...
void MainWindow::on_actionPlaybackCassette_triggered()
{
    ...
    playCassetteImage(fileName);
}

bool MainWindow::chooseCassetteEngine(const QString &fileName, CassettePlaybackEngine &engine)
{
    engine = CassettePlaybackStandard;

    bool containsFsk = false;
    QString errorMessage;
    if (!CassetteWorker::inspectCasImage(fileName, &containsFsk, &errorMessage)) {
    ...
    if (containsFsk) {
        QMessageBox::StandardButton answer = QMessageBox::question(
                    this,
                    tr("FSK cassette detected"),
                    tr("The selected cassette image contains FSK chunks.\n\n"
                       "Do you want to try loading it with the compatibility cassette engine?"),
                    QMessageBox::Yes | QMessageBox::No,
                    QMessageBox::Yes);
        if (answer != QMessageBox::Yes) {
            return false;
        }
        engine = CassettePlaybackFskCompatibility;
    }

    return true;
}

void MainWindow::playCassetteImage(const QString &fileName)
{
    CassettePlaybackEngine engine;
    if (!chooseCassetteEngine(fileName, engine)) {
        return;
    }
    ...
    CassetteDialog *dlg = new CassetteDialog(this, fileName, engine);
    dlg->exec();
    delete dlg;
