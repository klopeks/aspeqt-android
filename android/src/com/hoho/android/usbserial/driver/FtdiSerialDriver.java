...
public class FtdiSerialDriver implements UsbSerialDriver {
    ...
    public class FtdiSerialPort extends CommonUsbSerialPort {
        ...
        // bit masks for SIO_SET_MODEM_CTRL_REQUEST wValue:
        //   low byte (state):  bit0=DTR, bit1=RTS  (1=assert, 0=de-assert)
        //   high byte (mask):  bit0=affect DTR,    bit1=affect RTS
	...
	@Override
        public final int getStatus() throws IOException {
            byte[] buf = new byte[2];
            // FTDI returns 2 status bytes: [modemStatus, lineStatus]
            // modemStatus bits: 4=CTS, 5=DSR, 6=RI, 7=RLSD/DCD; bits 0-3 reserved
            // Original code was: do {...} while (result < 0); return buf[0]-1;
            // That had two bugs:
            //   1) infinite loop if controlTransfer keeps failing
            //   2) byte sign-extension: when DCD is high (bit 7), buf[0] is a
            //      negative Java byte, so buf[0]-1 is negative int. The caller
            //      treats negative as an I/O error and aborts the SIO loop.
            //      Fix: convert to unsigned (& 0xff) so 0..255 range.
            int retries = 0;
            int result;
            do {
                result = mConnection.controlTransfer(FTDI_DEVICE_IN_REQTYPE, SIO_POLL_MODEM_STATUS_REQUEST,
                        0, 0, buf, 2, USB_READ_TIMEOUT_MILLIS);
                if (result >= 2) break;
                if (++retries > 8) {
                    throw new IOException("getStatus controlTransfer failed: " + result);
                }
            } while (true);
            return buf[0] & 0xff;
        }
        ...
        // Local cached state — FTDI does not let us read DTR/RTS back, so we
        // remember what we last asserted.
        ...
        public void setDTR(boolean value) throws IOException {
            int wValue = value ? SIO_SET_DTR_HIGH : SIO_SET_DTR_LOW;
            int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE,
                    SIO_SET_MODEM_CTRL_REQUEST, wValue, 0, null, 0,
                    USB_WRITE_TIMEOUT_MILLIS);
            if (result < 0) {
                throw new IOException("setDTR failed: " + result);
            }
            mDtrState = value;
        }
        ...
        public void setRTS(boolean value) throws IOException {
            int wValue = value ? SIO_SET_RTS_HIGH : SIO_SET_RTS_LOW;
            int result = mConnection.controlTransfer(FTDI_DEVICE_OUT_REQTYPE,
                    SIO_SET_MODEM_CTRL_REQUEST, wValue, 0, null, 0,
                    USB_WRITE_TIMEOUT_MILLIS);
            if (result < 0) {
                throw new IOException("setRTS failed: " + result);
            }
            mRtsState = value;
        }
