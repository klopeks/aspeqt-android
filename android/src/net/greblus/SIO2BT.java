...
public class SIO2BT implements SerialDevice
{
    ...
    private boolean hasBluetoothConnect() {
        ...
        return sa.checkSelfPermission("android.permission.BLUETOOTH_CONNECT")
                == PackageManager.PERMISSION_GRANTED;
    }

    private void requestEnableViaIntent() {
        // BluetoothAdapter.enable() is no-op on Android 13+ and requires elevated
        // permission on 12. The supported flow is to launch ACTION_REQUEST_ENABLE
        // and let the system show the consent dialog.
        try {
            Intent enableIntent = new Intent(BluetoothAdapter.ACTION_REQUEST_ENABLE);
            sa.startActivity(enableIntent);
        } catch (Exception e) {
            Log.w("BT", "Failed to launch ACTION_REQUEST_ENABLE: " + e);
        }
    }

    SIO2BT() {
        BluetoothManager bm = (BluetoothManager) sa.getSystemService(Context.BLUETOOTH_SERVICE);
        m_BluetoothAdapter = (bm != null) ? bm.getAdapter() : BluetoothAdapter.getDefaultAdapter();

        if (m_BluetoothAdapter == null) {
            Toast.makeText(sa, net.greblus.LocaleHelper.getString(sa, R.string.bt_module_not_present), Toast.LENGTH_SHORT).show();
            return;
        }

        if (!m_BluetoothAdapter.isEnabled() && hasBluetoothConnect())
            requestEnableViaIntent();
    }
    ...
    public int openDevice() {
        ...
        if (!hasBluetoothConnect()) {
            sa.runOnUiThread(new Runnable() {
                public void run() {
                    Toast.makeText(sa, net.greblus.LocaleHelper.getString(sa, R.string.bt_module_check), Toast.LENGTH_LONG).show();
                }
            });
            return 0;
        }
        ...
        if (!m_BluetoothAdapter.isEnabled()) {
            requestEnableViaIntent();
            // Give the user a moment to confirm; bail out if still off.
            long deadline = System.currentTimeMillis() + 5000;
            while (!m_BluetoothAdapter.isEnabled() && System.currentTimeMillis() < deadline) {
                try { Thread.sleep(100); } catch (InterruptedException ie) {}
            }
            if (!m_BluetoothAdapter.isEnabled()) return 0;
        }

        try { m_BluetoothAdapter.cancelDiscovery(); } catch (SecurityException se) {}

        Set<BluetoothDevice> pairedDevices;
        try {
            pairedDevices = m_BluetoothAdapter.getBondedDevices();
        } catch (SecurityException se) {
            Log.w("BT", "BLUETOOTH_CONNECT denied: " + se);
            return 0;
        }
        ...
        if (m_device == null) {
            sa.runOnUiThread(new Runnable() {
                public void run() {
                    Toast.makeText(sa, net.greblus.LocaleHelper.getString(sa, R.string.bt_module_check), Toast.LENGTH_SHORT).show();
                }
            });
        return 0;
        }
        ...
        try {
                ...
                Log.w("BT", "createRfcommSocket denied: " + se);
                return 0;
        }
        m_socket = tmp;
        ...
        if (m_socket != null)
        {
            sa.runOnUiThread(new Runnable() {
                public void run() {
                    Toast.makeText(sa, net.greblus.LocaleHelper.getString(sa, R.string.bt_try_connecting), Toast.LENGTH_LONG).show();
                }
            });
            ...
            try {
                ...
                Log.w("BT", "socket.connect denied: " + se);
                return 0;
            }
        } else {
            return 0;
        ...
        if (m_socket.isConnected()) {
            ...
            if (debug)
                ...
                sa.runOnUiThread(new Runnable() {
                    public void run() {
                        Toast.makeText(sa, net.greblus.LocaleHelper.getString(sa, R.string.bt_connected), Toast.LENGTH_LONG).show();
                    }
                });
            return 1;
        } else {
            ...
            sa.runOnUiThread(new Runnable() {
                public void run() {
                    Toast.makeText(sa, net.greblus.LocaleHelper.getString(sa, R.string.bt_failed_connecting), Toast.LENGTH_LONG).show();
                }
            });
            return 0;
        }
