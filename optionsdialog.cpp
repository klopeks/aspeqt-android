    ...
    m_ui(new Ui::OptionsDialog)
{
    ...
    #ifdef Q_OS_ANDROID
    // Force-enable handshake combo when on USB SIO2PC. Some prior UI states
    // (or a stray currentIndexChanged on init from setCurrentIndex) can leave
    // it disabled, which made the dropdown unchangeable for users.
    if (aspeqtSettings->serialPortInterface() == 0 /* SIO2PC */) {
